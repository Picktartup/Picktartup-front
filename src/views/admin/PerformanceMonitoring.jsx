...
const useServiceMetrics = (serviceName, environment) => {
    ...
    useEffect(() => {
      const fetchMetrics = async () => {
        try {
          const prometheusEndpoint = 'http://192.168.0.141:30090';
  ...
  useEffect(() => {
    const fetchOverallMetrics = async () => {
      try {
        const responses = await Promise.all([
          // CPU 사용률 (예시)
          axios.get('http://192.168.0.141:30090/api/v1/query', {
            params: {
...
const ServiceCard = ({ service, environment }) => {
    ...
    const getGrafanaUrl = (service) => {
        const baseUrl = 'http://192.168.0.141:32647';
        const now = Date.now();
      ...
      {selectedEnvironment === 'all' && (
        <>
          ...
          <div className="h-[600px]">
            <iframe
              src="http://192.168.0.141:32647/d/ff635a025bcfea7bc3dd4f508990a3e9/kubernetes-networking-cluster?orgId=1&from=2024-12-06T09:16:21.506Z&to=2024-12-06T10:16:21.507Z&timezone=utc&var-datasource=default&var-cluster=&refresh=10s&kiosk"
